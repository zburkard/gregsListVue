<template>
  ...
  <div class="cols" @click="goTo">
    <div class="col" ontouchstart="this.classList.toggle('hover');">
      <div class="container">
        <div class="front" :style="`background-image: url(${car.imgUrl})`">
          <div class="inner">
            <p>{{car.year}}</p><p>{{ car.make }}</p><p>{{ car.model }}</p>
          </div>
        ...
        <div class="back">
          <div class="inner">
            ...
            <button v-if="(car.creatorId == account.id)" @click.stop="removeCar(car.id)"
              class="btn btn-danger delete-btn rounded-pill"><i class="  px-2 mdi mdi-delete-forever"></i></button>
            <div v-if="car.creator" class="creator p-1 d-flex justify-content-center">
              <h5>Posted By: {{ car.creator.name }}</h5>
              <img :src="car.creator.picture" alt="">
            </div>
          </div>
        </div>
